...
const state = {
  ...
  profile:store.get("profile",{name:""}), logs:store.get("logs",[])
};
...
function tasksHTML(){ return `<main class="app-content"><h2>المهام</h2><form data-task-form class="task-form"><input name="text" placeholder="أضف مهمة جديدة…" required><input name="due" type="date" aria-label="الموعد"><select name="priority" aria-label="الأولوية"><option value="normal">عادية</option><option value="high">عالية</option><option value="low">منخفضة</option></select><button class="primary">إضافة</button></form><div data-task-list></div></main>`; }
function renderTasks(win){ $("[data-task-list]",win).innerHTML=state.tasks.length?state.tasks.map(t=>`<label class="file-card" style="display:flex;grid-template-columns:auto 1fr auto;align-items:center"><input type="checkbox" data-task-toggle="${t.id}" ${t.done?"checked":""}><span style="${t.done?"text-decoration:line-through;color:var(--muted)":""}">${t.priority==="high"?"● ":""}${escapeHTML(t.text)}${t.due?`<small style="display:block">${escapeHTML(t.due)}</small>`:""}</span><button data-task-delete="${t.id}" aria-label="حذف">×</button></label>`).join(""):"<p style='color:var(--muted)'>لا توجد مهام. يوم جديد، بداية مرتبة.</p>"; }
...
function aboutHTML(){ return `<main class="app-content"><div class="about-logo"><div class="brand-mark">ن</div><h2>NourBest OS 2.2</h2><p>مساحة عمل عربية هجينة تعمل كتطبيق PWA على المنصات المقيدة، وكواجهة محلية مستضافة داخل إصدار Nour Linux القابل للبناء للحواسيب.</p><table class="spec-table"><tr><td>الإصدار</td><td>2.2 Universal</td></tr><tr><td>الواجهة</td><td>Nour Shell</td></tr><tr><td>الملفات</td><td>IndexedDB مع استيراد وتصدير</td></tr><tr><td>الاتصال</td><td>يعمل دون إنترنت</td></tr><tr><td>الترخيص</td><td>MIT</td></tr></table></div></main>`; }
...
async function boot(){
  if(window.NourDB){try{const saved=await NourDB.hydrate(["notes","tasks","files","profile","logs"]);if(saved.notes)state.notes=saved.notes;if(saved.tasks)state.tasks=saved.tasks;if(saved.files)state.files=saved.files;if(saved.profile)state.profile=saved.profile;if(saved.logs)state.logs=saved.logs;}catch(error){systemLog("database",error.message);}}
  store.set("schemaVersion",2);renderShell();bindGlobal();tick();setInterval(tick,1000);renderCalendar();
